...
NaraeClaw supports automatic failover to alternative providers when the primary encounters:
...
When a provider returns 429 (rate limit), NaraeClaw:
...
| `minimax` | `minimax-intl`, `minimax-io`, `minimax-global`, `minimax-oauth`, `minimax-portal` | No | `MINIMAX_OAUTH_TOKEN`, `MINIMAX_API_KEY` |
...
- After multimodal normalization, NaraeClaw sends image payloads through Ollama's native `messages[].images` field.
- If a non-vision provider is selected, NaraeClaw returns a structured capability error instead of silently ignoring images.
...
- NaraeClaw normalizes a trailing `/api` in `api_url` automatically.
...
- Model discovery: `naraeclaw models refresh --provider llamacpp`
...
- Model discovery: `naraeclaw models refresh --provider sglang`
...
- Model discovery: `naraeclaw models refresh --provider vllm`
...
- Model discovery: `naraeclaw models refresh --provider osaurus`
...
- Model discovery: `naraeclaw models refresh --provider nvidia`
