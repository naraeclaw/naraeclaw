# NaraeClaw 문서

최종 갱신: **2026-04-13**

## 바로 찾기

| 하고 싶은 것 | 읽을 문서 |
|---|---|
| 빠르게 설치·실행 | [README.md](../README.md) |
| 온보딩 마법사 실행 | `naraeclaw onboard` |
| CLI 명령어 전체 조회 | [commands-reference.md](reference/cli/commands-reference.md) |
| 설정 키·기본값 확인 | [config-reference.md](reference/api/config-reference.md) |
| 운영 런북 (day-2) | [operations-runbook.md](ops/operations-runbook.md) |
| 장애·설치 문제 해결 | [troubleshooting.md](ops/troubleshooting.md) |
| 카테고리별 전체 목록 | [SUMMARY.md](SUMMARY.md) |

## 빠른 판단 트리

- 처음 설치? → [setup-guides/README.md](setup-guides/README.md)
- CLI/설정 키 확인? → [reference/README.md](reference/README.md)
- 운영·서비스 배포? → [ops/README.md](ops/README.md)
- 장애·회귀 분석? → [troubleshooting.md](ops/troubleshooting.md)
- 보안 강화? → [security/README.md](security/README.md)
- 기여·CI 워크플로우? → [contributing/README.md](contributing/README.md)
- 전체 목차? → [SUMMARY.md](SUMMARY.md)

## 독자별 안내

### 사용자 / 운영자

- [commands-reference.md](reference/cli/commands-reference.md) — 워크플로우별 명령어
- [providers-reference.md](reference/api/providers-reference.md) — Provider ID, 별칭, 환경변수
- [channels-reference.md](reference/api/channels-reference.md) — 채널 기능 및 설정 경로
- [config-reference.md](reference/api/config-reference.md) — 주요 설정 키 및 보안 기본값
- [operations-runbook.md](ops/operations-runbook.md) — day-2 운영 및 롤백 절차
- [troubleshooting.md](ops/troubleshooting.md) — 장애 패턴 및 복구 방법

### 기여자 / 메인테이너

- [../CONTRIBUTING.md](../CONTRIBUTING.md)
- [contributing/pr-workflow.md](contributing/pr-workflow.md)
- [contributing/reviewer-playbook.md](contributing/reviewer-playbook.md)

### 보안 / 안정성

- [security/README.md](security/README.md)
- [security/sandboxing.md](security/sandboxing.md)
- [security/audit-logging.md](security/audit-logging.md)
- [security/security-roadmap.md](security/security-roadmap.md)
- [ops/resource-limits.md](ops/resource-limits.md)
