...
NaraeClaw supports custom API endpoints for both OpenAI-compatible and Anthropic-compatible providers.
...
NaraeClaw includes a first-class local provider for `llama-server`:
...
```bash
naraeclaw models refresh --provider llamacpp
...
NaraeClaw includes a first-class local provider for [SGLang](https://github.com/sgl-project/sglang):
...
```bash
naraeclaw models refresh --provider sglang
naraeclaw agent -m "hello"
```
...
NaraeClaw includes a first-class local provider for [vLLM](https://docs.vllm.ai/):
...
```bash
naraeclaw models refresh --provider vllm
naraeclaw agent -m "hello"
```
...
# Single message test
naraeclaw agent -m "test message"
```
