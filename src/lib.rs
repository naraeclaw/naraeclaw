...
Examples:
  naraeclaw gateway start              # use config defaults
  naraeclaw gateway start -p 8080      # listen on port 8080
  naraeclaw gateway start --host 0.0.0.0   # requires [gateway].allow_public_bind=true or a tunnel
  naraeclaw gateway start -p 0         # random available port")]
    Start {
...
Examples:
  naraeclaw gateway restart            # restart with config defaults
  naraeclaw gateway restart -p 8080    # restart on port 8080")]
    Restart {
        /// Port to listen on (use 0 for random available port); defaults to config gateway.port
        #[arg(short, long)]
        port: Option<u16>,
...
Examples:
  naraeclaw gateway get-paircode       # show current pairing code
  naraeclaw gateway get-paircode --new # generate a new pairing code")]
    GetPaircode {
...
Examples:
  naraeclaw cron add '0 9 * * 1-5' 'Good morning' --tz America/New_York --agent
  naraeclaw cron add '*/30 * * * *' 'Check system health' --agent
...
Examples:
  naraeclaw cron add-at 2025-01-15T14:00:00Z 'Send reminder'
  naraeclaw cron add-at 2025-12-31T23:59:00Z 'Happy New Year!'")]
    AddAt {
...
Examples:
  naraeclaw cron add-every 60000 'Ping heartbeat'     # every minute
  naraeclaw cron add-every 3600000 'Hourly report'    # every hour")]
    AddEvery {
...
Examples:
  naraeclaw cron once 30m 'Run backup in 30 minutes'
  naraeclaw cron once 2h 'Follow up on deployment'
  naraeclaw cron once 1d 'Daily check'")]
    Once {
        /// Delay duration
        delay: String,
        /// Treat the argument as an agent prompt instead of a shell command
...
Examples:
  naraeclaw cron update <task-id> --expression '0 8 * * *'
  naraeclaw cron update <task-id> --tz Europe/London --name 'Morning check'
  naraeclaw cron update <task-id> --command 'Updated message'")]
    Update {
        /// Task ID
        id: String,
        /// New cron expression
...
Examples:
  ...
  naraeclaw hardware introspect COM3")]
    Introspect {
...
Examples:
  ...
  naraeclaw peripheral add esp32 /dev/ttyUSB0")]
    Add {
    ...
    /// Flash NaraeClaw firmware to Arduino (creates .ino, installs arduino-cli if needed, uploads)
    #[command(long_about = "\
Flash NaraeClaw firmware to an Arduino board.

Generates the .ino sketch, installs arduino-cli if it is not \
already available, compiles, and uploads the firmware.

Examples:
  naraeclaw peripheral flash
  naraeclaw peripheral flash --port /dev/cu.usbmodem12345
  naraeclaw peripheral flash -p COM3")]
    Flash {
    ...
    /// Flash NaraeClaw firmware to Nucleo-F401RE (builds + probe-rs run)
    FlashNucleo,
}
