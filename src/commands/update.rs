...
pub async fn check(target_version: Option<&str>) -> Result<UpdateInfo> {
    ...
    let client = reqwest::Client::builder()
        .user_agent(format!("naraeclaw/{current}"))
...
pub async fn run(target_version: Option<&str>) -> Result<()> {
    ...
    let download_path = temp_dir.path().join("naraeclaw_new");
...
async fn download_binary(url: &str, dest: &Path) -> Result<()> {
    let client = reqwest::Client::builder()
        .user_agent(format!("naraeclaw/{}", env!("CARGO_PKG_VERSION")))
...
fn extract_tar_gz(archive_bytes: &[u8], dest: &Path) -> Result<()> {
    ...
    for entry in archive.entries().context("failed to read tar entries")? {
        ...
        // The archive contains a single binary named "naraeclaw" (or "naraeclaw.exe" on Windows).
        let file_name = path.file_name().and_then(|n| n.to_str()).unwrap_or("");

        if file_name == "naraeclaw" || file_name == "naraeclaw.exe" {
            let mut buf = Vec::new();
            entry
                .read_to_end(&mut buf)
    ...
    bail!("archive does not contain a 'naraeclaw' binary")
}
...
async fn validate_binary(path: &Path) -> Result<()> {
    ...
    if !stdout.contains("naraeclaw") {
        bail!("downloaded binary does not appear to be naraeclaw");
    }
...
mod tests {
    ...
    fn find_asset_url_picks_correct_gnu_over_other_linux_assets() {
        let release = make_release(&[
            ...
            "naraeclaw-aarch64-apple-darwin.tar.gz",
        ]);
    ...
    fn extract_tar_gz_finds_binary() {
        ...
        // Build a tar.gz in memory containing a fake "naraeclaw" binary.
        ...
        let mut tar_buf = Vec::new();
        {
            ...
            header.set_cksum();
            builder
                .append_data(&mut header, "naraeclaw", &fake_binary[..])
                .unwrap();
        ...
        let dest = tmp.path().join("naraeclaw_extracted");
    ...
    fn extract_tar_gz_errors_on_missing_binary() {
        ...
        // Build a tar.gz with a file that is NOT named "naraeclaw".
        let mut tar_buf = Vec::new();
        {
            let mut builder = tar::Builder::new(&mut tar_buf);
            let mut header = tar::Header::new_gnu();
