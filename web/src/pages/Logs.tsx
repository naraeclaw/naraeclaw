...
const LOG_STORAGE_KEY = 'naraeclaw_live_logs';
const LEGACY_LOG_STORAGE_KEY = 'zeroclaw_live_logs';
const MAX_PERSISTED_LOGS = 200;

function loadPersistedLogs(): LogEntry[] {
  try {
    let raw = sessionStorage.getItem(LOG_STORAGE_KEY);
    if (!raw) {
      raw = sessionStorage.getItem(LEGACY_LOG_STORAGE_KEY);
      if (raw) {
        sessionStorage.setItem(LOG_STORAGE_KEY, raw);
        sessionStorage.removeItem(LEGACY_LOG_STORAGE_KEY);
      }
    }
    if (!raw) return [];
      ...
      {!infoDismissed && (
        <div className="flex items-start gap-3 px-6 py-3 border-b flex-shrink-0" style={{ borderColor: 'rgba(56, 189, 248, 0.2)', background: 'rgba(56, 189, 248, 0.05)' }}>
          <div className="flex-1 text-xs" style={{ color: 'var(--pc-text-secondary)' }}>
            ...
            {' '}To see tracing logs, run the daemon with <code>RUST_LOG=info naraeclaw</code> and check your terminal.
          </div>
          <button
