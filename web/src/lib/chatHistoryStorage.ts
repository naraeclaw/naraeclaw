...
const PREFIX = 'naraeclaw_chat_history_v1:';
...
export function loadChatHistory(sessionId: string): PersistedChatBubble[] {
  try {
    let raw = localStorage.getItem(storageKey(sessionId));
    if (!raw) {
      raw = localStorage.getItem(legacyStorageKey(sessionId));
      if (raw) {
        localStorage.setItem(storageKey(sessionId), raw);
        localStorage.removeItem(legacyStorageKey(sessionId));
      }
    }
    if (!raw) return [];
