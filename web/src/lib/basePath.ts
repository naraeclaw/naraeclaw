...
declare global {
  interface Window {
    __NARAECLAW_BASE__?: string;
    __ZEROCLAW_BASE__?: string;
  }
}

/** Gateway path prefix (e.g. "/naraeclaw"), or empty string when served at root. */
export const basePath: string = isTauri()
  ? ''
  : (window.__NARAECLAW_BASE__ ?? window.__ZEROCLAW_BASE__ ?? '').replace(/\/+$/, '');
