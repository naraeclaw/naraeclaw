...
**NaraeClaw**는 [ZeroClaw](https://github.com/zeroclaw-labs/zeroclaw)에서 출발한 포크로, macOS·Windows·Linux에서 동작하는 한국어 우선 경량 AI 에이전트 런타임입니다. CLI + 게이트웨이 API를 핵심 표면으로 두고, 레거시 범위는 단계적으로 정리합니다.

현재 중점:
- ✅ **브랜딩 정리** — 사용자 노출 문구를 NaraeClaw 기준으로 통일
- ✅ **경량화** — Desktop(`apps/tauri/`), Web(`web/`) 제거 → CLI + 게이트웨이 단일화
...
Rust edition 2024 워크스페이스. `naraeclaw` 바이너리(`src/main.rs`)는 기본적으로 `agent-runtime` feature가 켜져 있고, CLI + 게이트웨이 서브시스템을 활성화합니다.
...
- `crates/naraeclaw-gateway/src/` — HTTP/WebSocket 게이트웨이
...
핵심 작업은 모두 완료됨. 현재 우선순위:

1. **핵심 경로 안정화** — CLI / 게이트웨이의 기본 실행, 설정, 종료, 복원 흐름 검증
2. **브랜딩 정리** — 사용자 노출 문자열을 NaraeClaw 기준으로 통일
3. **검증 단순화** — fmt / check / clippy / 핵심 테스트를 빠르게 돌리는 루프 유지
