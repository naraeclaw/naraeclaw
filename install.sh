...
TOML
  ...
  # --- Workspace scaffold ---
  local subdirs=(sessions state cron skills skills/byoridb-memory)
...
- Use byoridb__memory_remember for one standalone durable fact.
- Use byoridb__memory_wiki_upsert plus byoridb__memory_link when relationships matter.
- Record knowledge at meaningful checkpoints, not after every conversational turn.
- Never store secrets, credentials, tokens, or transient chatter.

---
*Add your own conventions, style, and rules.*"

  local byoridb_skill_path="$workspace_dir/skills/byoridb-memory/SKILL.md"
  if [[ ! -f "$byoridb_skill_path" ]]; then
    cat > "$byoridb_skill_path" <<'BYORIDB_SKILL'
---
name: byoridb-memory
