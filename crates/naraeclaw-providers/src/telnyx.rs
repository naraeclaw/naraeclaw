...
///
/// ```rust,ignore
/// use naraeclaw::providers::telnyx::TelnyxProvider;
/// use naraeclaw::providers::Provider;
///
...
impl TelnyxProvider {
    ...
    /// 2. `NARAECLAW_API_KEY` environment variable (fallback; `ZEROCLAW_API_KEY` remains supported)
...
impl Provider for TelnyxProvider {
    async fn chat_with_system(
        ...
        let api_key = self.api_key.as_ref().ok_or_else(|| {
            anyhow::anyhow!(
                "Telnyx API key not set. Set TELNYX_API_KEY environment variable or run `naraeclaw onboard`."
            )
        })?;
