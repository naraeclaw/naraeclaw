...
impl OpenAiCompatibleProvider {
    ...
    fn http_client(&self) -> Client {
        ...
        if has_user_agent || has_extra_headers {
            ...
            let builder = naraeclaw_config::schema::apply_runtime_proxy_to_builder(
                builder,
                "provider.compatible",
            );
        ...
        naraeclaw_config::schema::build_runtime_proxy_client_with_timeouts(
            "provider.compatible",
            timeout,
            10,
        )
    ...
    fn tool_specs_to_openai_format(
        tools: &[naraeclaw_api::tool::ToolSpec],
    ) -> Vec<serde_json::Value> {
        tools
            .iter()
            .map(|tool| {
                let params =
                    naraeclaw_api::schema::SchemaCleanr::clean_for_openai(tool.parameters.clone());
                serde_json::json!({
                    "type": "function",
                    "function": {
                        "name": tool.name,
...
impl OpenAiCompatibleProvider {
    ...
    fn convert_tool_specs(
        tools: Option<&[naraeclaw_api::tool::ToolSpec]>,
    ) -> Option<Vec<serde_json::Value>> {
        tools.map(|items| {
            items
                .iter()
                .map(|tool| {
                    let params = naraeclaw_api::schema::SchemaCleanr::clean_for_openai(
    ...
    fn with_prompt_guided_tool_instructions(
        ...
        let instructions = naraeclaw_api::provider::build_tool_instructions_text(tools);
...
impl Provider for OpenAiCompatibleProvider {
    fn capabilities(&self) -> naraeclaw_api::provider::ProviderCapabilities {
        naraeclaw_api::provider::ProviderCapabilities {
...
mod tests {
    ...
    fn request_serializes_correctly() {
        let req = ApiChatRequest {
            ...
            messages: vec![
                Message {
                    role: "system".to_string(),
                    content: MessageContent::Text("You are NaraeClaw".to_string()),
                },
                Message {
                    role: "user".to_string(),
    ...
    fn prompt_guided_tool_fallback_injects_system_instruction() {
        ...
        let tools = vec![naraeclaw_api::tool::ToolSpec {
    ...
    fn tool_specs_convert_to_openai_format() {
        let specs = vec![naraeclaw_api::tool::ToolSpec {
