...
impl Provider for AnthropicProvider {
    ...
    async fn chat(
        ...
        let tool_choice_override = naraeclaw_api::TOOL_CHOICE_OVERRIDE
...
mod tests {
    ...
    async fn chat_with_system_fails_without_key() {
        let p = AnthropicProvider::new(None);
        let result = p
            .chat_with_system(Some("You are NaraeClaw"), "hello", "claude-3-opus", 0.7)
            .await;
        assert!(result.is_err());
    }
    ...
    fn chat_request_serializes_with_system() {
        let req = ChatRequest {
            model: "claude-3-opus".to_string(),
            max_tokens: 4096,
            system: Some("You are NaraeClaw".to_string()),
            messages: vec![Message {
                role: "user".to_string(),
                content: "hello".to_string(),
            }],
        ...
        let json = serde_json::to_string(&req).unwrap();
        assert!(json.contains("\"system\":\"You are NaraeClaw\""));
    }
