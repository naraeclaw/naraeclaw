...
/// - `X-NaraeClaw-Timestamp` — unix epoch seconds
/// - `X-NaraeClaw-Nonce` — random UUID v4
/// - `X-NaraeClaw-Signature` — HMAC-SHA256 hex digest
///
...
impl NodeTransport {
    ...
    pub async fn send(
        ...
        let resp = self
            .http
            .post(&url)
            .header("X-NaraeClaw-Timestamp", timestamp.to_string())
            .header("X-NaraeClaw-Nonce", &nonce)
            .header("X-NaraeClaw-Signature", &signature)
