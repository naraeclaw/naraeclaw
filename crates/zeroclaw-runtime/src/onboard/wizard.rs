...
pub async fn run_wizard(force: bool, callbacks: WizardCallbacks) -> Result<Config> {
    ...
    let config = Config {
        ...
        cron: zeroclaw_config::schema::CronConfig::default(),
        channels_config,
        ...
        http_request: zeroclaw_config::schema::HttpRequestConfig::default(),
        multimodal: zeroclaw_config::schema::MultimodalConfig::default(),
        media_pipeline: zeroclaw_config::schema::MediaPipelineConfig::default(),
        web_fetch: zeroclaw_config::schema::WebFetchConfig::default(),
        ...
        linkedin: zeroclaw_config::schema::LinkedInConfig::default(),
        image_gen: zeroclaw_config::schema::ImageGenConfig::default(),
        plugins: zeroclaw_config::schema::PluginsConfig::default(),
        locale: None,
        verifiable_intent: zeroclaw_config::schema::VerifiableIntentConfig::default(),
        claude_code: zeroclaw_config::schema::ClaudeCodeConfig::default(),
        claude_code_runner: zeroclaw_config::schema::ClaudeCodeRunnerConfig::default(),
        codex_cli: zeroclaw_config::schema::CodexCliConfig::default(),
        gemini_cli: zeroclaw_config::schema::GeminiCliConfig::default(),
...
fn memory_config_defaults_for_backend(backend: &str) -> MemoryConfig {
    ...
    MemoryConfig {
        ...
        vector_weight: 0.7,
        keyword_weight: 0.3,
        search_mode: zeroclaw_config::schema::SearchMode::default(),
...
fn resolve_quick_setup_dirs_with_home(home: &Path) -> (PathBuf, PathBuf) {
        ...
        std::env::var("NARAECLAW_WORKSPACE").or_else(|_| std::env::var("ZEROCLAW_WORKSPACE"))
    {
        ...
        if !trimmed.is_empty() {
            ...
            return zeroclaw_config::schema::resolve_config_dir_for_workspace(&PathBuf::from(
                expanded.as_ref(),
            ));
        }
    }
...
pub async fn run_models_refresh_all(config: &Config, force: bool) -> Result<()> {
    let mut targets: Vec<String> = zeroclaw_providers::list_providers()
        .into_iter()
...
async fn persist_workspace_selection(config_path: &Path) -> Result<()> {
    ...
    zeroclaw_config::schema::persist_active_workspace_config_dir(config_dir)
        .await
        .with_context(|| {
            format!(
...
async fn setup_workspace() -> Result<(PathBuf, PathBuf)> {
    let (default_config_dir, default_workspace_dir) =
        zeroclaw_config::schema::resolve_runtime_dirs_for_onboarding().await?;
    ...
    let (config_dir, workspace_dir) = if use_default {
        ...
        zeroclaw_config::schema::resolve_config_dir_for_workspace(&PathBuf::from(expanded))
    };
...
async fn setup_provider(workspace_dir: &Path) -> Result<(String, String, String, Option<String>)> {
    ...
    let api_key = if provider_name == "ollama" {
        ...
        if zeroclaw_providers::gemini::GeminiProvider::has_cli_credentials() {
            print_bullet(&format!(
...
#[allow(clippy::too_many_lines)]
fn setup_tunnel() -> Result<zeroclaw_config::schema::TunnelConfig> {
    use zeroclaw_config::schema::{
...
mod tests {
    ...
    fn matrix_fresh_install_uses_defaults_for_non_prompted_fields() {
        ...
        let existing_mx: Option<&zeroclaw_config::schema::MatrixConfig> = None;
