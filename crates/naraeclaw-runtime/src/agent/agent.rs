...
use naraeclaw_memory::{self, Memory, MemoryCategory};
use naraeclaw_providers::{self, ChatMessage, ChatRequest, ConversationMessage, Provider};
...
pub use naraeclaw_api::agent::TurnEvent;
...
pub struct Agent {
    ...
    config: naraeclaw_config::schema::AgentConfig,
    model_name: String,
    temperature: f64,
    workspace_dir: std::path::PathBuf,
    identity_config: naraeclaw_config::schema::IdentityConfig,
    skills: Vec<crate::skills::Skill>,
    skills_prompt_mode: naraeclaw_config::schema::SkillsPromptInjectionMode,
    auto_save: bool,
    memory_session_id: Option<String>,
    history: Vec<ConversationMessage>,
    classification_config: naraeclaw_config::schema::QueryClassificationConfig,
    available_hints: Vec<String>,
    route_model_by_hint: HashMap<String, String>,
    #[allow(dead_code)] // WIP: stored for future runtime tool filtering
    allowed_tools: Option<Vec<String>>,
    response_cache: Option<Arc<naraeclaw_memory::response_cache::ResponseCache>>,
...
pub struct AgentBuilder {
    ...
    config: Option<naraeclaw_config::schema::AgentConfig>,
    model_name: Option<String>,
    temperature: Option<f64>,
    workspace_dir: Option<std::path::PathBuf>,
    identity_config: Option<naraeclaw_config::schema::IdentityConfig>,
    skills: Option<Vec<crate::skills::Skill>>,
    skills_prompt_mode: Option<naraeclaw_config::schema::SkillsPromptInjectionMode>,
    auto_save: Option<bool>,
    memory_session_id: Option<String>,
    classification_config: Option<naraeclaw_config::schema::QueryClassificationConfig>,
...
impl AgentBuilder {
    ...
    pub fn config(mut self, config: naraeclaw_config::schema::AgentConfig) -> Self {
    ...
    pub fn response_cache(
        mut self,
        cache: Option<Arc<naraeclaw_memory::response_cache::ResponseCache>>,
    ) -> Self {
...
impl Agent {
    ...
    pub async fn from_config(config: &Config) -> Result<Self> {
        ...
        let provider: Box<dyn Provider> = naraeclaw_providers::create_routed_provider_with_options(
            provider_name,
            config.api_key.as_deref(),
        ...
        let response_cache = if config.memory.response_cache_enabled {
            naraeclaw_memory::response_cache::ResponseCache::with_hot_cache(
                &config.workspace_dir,
            ...
            let cache_key = if self.temperature == 0.0 {
                self.response_cache.as_ref().map(|_| {
                    ...
                    let system = messages
                        ...
                        .map(|m| m.content.as_str());
                    naraeclaw_memory::response_cache::ResponseCache::cache_key(
                        &effective_model,
                        system,
                        last_user,
                    )
    ...
    pub async fn turn_streamed(
        ...
        for _ in 0..self.config.max_tool_iterations {
            ...
            let stream_opts = naraeclaw_providers::traits::StreamOptions::new(true);
            let mut stream = self.provider.stream_chat(
                naraeclaw_providers::ChatRequest {
                    messages: &messages,
                    tools: if self.tool_dispatcher.should_send_tool_specs() {
                        Some(&self.tool_specs)
                    } else {
            ...
            let mut streamed_tool_calls: Vec<naraeclaw_providers::traits::ToolCall> = Vec::new();
            let mut got_stream = false;

            while let Some(item) = stream.next().await {
                match item {
                    Ok(event) => match event {
                        naraeclaw_providers::traits::StreamEvent::TextDelta(chunk) => {
                        ...
                        naraeclaw_providers::traits::StreamEvent::ToolCall(tc) => {
                            got_stream = true;
                            let _ = event_tx
                        ...
                        naraeclaw_providers::traits::StreamEvent::PreExecutedToolCall {
                            name,
                            args,
                        } => {
                            let _ = event_tx
                        ...
                        naraeclaw_providers::traits::StreamEvent::PreExecutedToolResult {
                            name,
                            output,
                        } => {
                            let _ = event_tx.send(TurnEvent::ToolResult { name, output }).await;
                        }
                        naraeclaw_providers::traits::StreamEvent::Final => break,
                    },
                    Err(_) => break,
                }
            }
            ...
            let response = if got_stream {
                ...
                naraeclaw_providers::ChatResponse {
    ...
    pub async fn run_interactive(&mut self) -> Result<()> {
        ...
        let listen_handle = tokio::spawn(async move {
            let _ = naraeclaw_api::channel::Channel::listen(&*cli, tx).await;
        });
...
mod tests {
    ...
    struct MockProvider {
        responses: Mutex<Vec<naraeclaw_providers::ChatResponse>>,
    }
    ...
    impl Provider for MockProvider {
        ...
        async fn chat(
            ...
            _model: &str,
            _temperature: f64,
        ) -> Result<naraeclaw_providers::ChatResponse> {
            let mut guard = self.responses.lock();
            if guard.is_empty() {
                return Ok(naraeclaw_providers::ChatResponse {
                    text: Some("done".into()),
                    tool_calls: vec![],
                    usage: None,
                    reasoning_content: None,
    ...
    async fn turn_without_tools_returns_text() {
        let provider = Box::new(MockProvider {
            responses: Mutex::new(vec![naraeclaw_providers::ChatResponse {
        ...
        let memory_cfg = naraeclaw_config::schema::MemoryConfig {
            backend: "none".into(),
            ..naraeclaw_config::schema::MemoryConfig::default()
        };
        let mem: Arc<dyn Memory> = Arc::from(
            naraeclaw_memory::create_memory(&memory_cfg, std::path::Path::new("/tmp"), None)
        ...
        let provider = Box::new(MockProvider {
            responses: Mutex::new(vec![
                naraeclaw_providers::ChatResponse {
                    ...
                    tool_calls: vec![naraeclaw_providers::ToolCall {
                        id: "tc1".into(),
                        name: "echo".into(),
                        arguments: "{}".into(),
                    }],
    ...
    impl Provider for StreamToolCaptureProvider {
        ...
        async fn chat(
            ...
            *count += 1;
            if *count == 1 {
                Ok(naraeclaw_providers::ChatResponse {
                    text: Some(String::new()),
                    tool_calls: vec![naraeclaw_providers::ToolCall {
                        id: "tc_stream_1".into(),
                        name: "echo".into(),
        ...
        fn stream_chat(
            ...
            _model: &str,
            _temperature: f64,
            _options: naraeclaw_providers::traits::StreamOptions,
        ) -> futures_util::stream::BoxStream<
            'static,
            naraeclaw_providers::traits::StreamResult<naraeclaw_providers::traits::StreamEvent>,
        > {
            ...
            *count += 1;
            if *count == 1 {
                let tc = naraeclaw_providers::traits::StreamEvent::ToolCall(
                    naraeclaw_providers::ToolCall {
                        id: "tc_stream_1".into(),
                        name: "echo".into(),
                        arguments: "{}".into(),
                    },
                ...
                stream::iter(vec![
                    Ok(tc),
                    Ok(naraeclaw_providers::traits::StreamEvent::Final),
                ])
                .boxed()
            } else {
                let chunk = naraeclaw_providers::traits::StreamEvent::TextDelta(
                    naraeclaw_providers::traits::StreamChunk {
    ...
    fn trim_history_does_not_leave_orphan_tool_results() {
        use naraeclaw_providers::{ToolCall, ToolResultMessage};
        ...
        let agent_config = naraeclaw_config::schema::AgentConfig {
            max_history_messages: 4,
            ..naraeclaw_config::schema::AgentConfig::default()
        };
