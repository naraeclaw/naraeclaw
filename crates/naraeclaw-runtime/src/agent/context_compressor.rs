...
use naraeclaw_api::provider::{ChatMessage, Provider};
use naraeclaw_memory::traits::Memory;

pub use naraeclaw_config::scattered_types::ContextCompressionConfig;
        ...
        if let Some(ref memory) = self.memory {
            ...
            if let Err(e) = memory
                .store(
                    &facts_key,
                    &summary,
                    naraeclaw_memory::traits::MemoryCategory::Daily,
                    None,
                )
                .await
            {
