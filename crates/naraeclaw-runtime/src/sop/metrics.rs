...
use naraeclaw_memory::traits::{Memory, MemoryCategory};
...
mod tests {
    ...
    async fn warm_start_roundtrip() {
        let mem_cfg = naraeclaw_config::schema::MemoryConfig {
            backend: "sqlite".into(),
            ..naraeclaw_config::schema::MemoryConfig::default()
        };
        let tmp = tempfile::tempdir().unwrap();
        let memory: std::sync::Arc<dyn Memory> = std::sync::Arc::from(
            naraeclaw_memory::create_memory(&mem_cfg, tmp.path(), None).unwrap(),
        );
