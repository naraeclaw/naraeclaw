                    ...
                    Ok(entries) if !entries.is_empty() => {
                        let ctx: String = entries
                            ...
                            .filter(|e| {
                                !matches!(
                                    e.category,
                                    naraeclaw_memory::traits::MemoryCategory::Conversation
                                )
                            })
                        ...
                        && let Some(ref mem) = heartbeat_memory
                    {
                        ...
                        let _ = mem
                            .store(
                                ...
                                naraeclaw_memory::MemoryCategory::Daily,
                                None,
                            )
                            .await;
                    }
...
fn load_heartbeat_session_context(config: &Config) -> Option<String> {
    use naraeclaw_providers::traits::ChatMessage;
...
fn load_jsonl_messages(path: &std::path::Path) -> Vec<naraeclaw_providers::traits::ChatMessage> {
    ...
    let mut window: VecDeque<naraeclaw_providers::traits::ChatMessage> =
    ...
    for line in reader.lines() {
        ...
        if let Ok(msg) = serde_json::from_str::<naraeclaw_providers::traits::ChatMessage>(trimmed) {
...
mod tests {
    ...
    fn detects_nextcloud_talk_as_supervised_channel() {
        ...
        config.channels_config.nextcloud_talk =
            Some(naraeclaw_config::schema::NextcloudTalkConfig {
                enabled: true,
