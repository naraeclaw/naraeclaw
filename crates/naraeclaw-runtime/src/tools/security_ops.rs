...
use naraeclaw_config::schema::SecurityOpsConfig;
    ...
    fn generate_report(&self, args: &serde_json::Value) -> anyhow::Result<ToolResult> {
        ...
        let report = format!(
             ...
             **Generated:** {}\n\n\
             ...
             ---\n\
             *Report generated by NaraeClaw MCSS Agent*\n",
...
mod tests {
    ...
    async fn generate_report_produces_markdown() {
        ...
        assert!(result.success);
        assert!(result.output.contains("NaraeClaw Corp"));
