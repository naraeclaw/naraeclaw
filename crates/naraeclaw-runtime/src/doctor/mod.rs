...
pub fn run(config: &Config) -> Result<()> {
    ...
    // Print report
    println!("🩺 NaraeClaw Doctor (enhanced)");
    println!();
    ...
    if errors > 0 {
        println!("  💡 Fix the errors above, then run `naraeclaw doctor` again.");
    }
...
fn doctor_model_targets(provider_override: Option<&str>) -> Vec<String> {
    ...
    naraeclaw_providers::list_providers()
        .into_iter()
...
pub async fn run_models(
    ...
    println!("🩺 NaraeClaw Doctor — Model Catalog Probe");
...
pub fn run_traces(
    ...
    println!();
    println!("Use `naraeclaw doctor traces --id <trace-id>` to inspect a full event payload.");
    Ok(())
}
...
fn check_config_semantics(config: &Config, items: &mut Vec<DiagItem>) {
    ...
    if has_channel {
        ...
        items.push(DiagItem::warn(
            cat,
            "no channels configured — run `naraeclaw onboard` to set one up",
        ));
    }
...
fn provider_validation_error(name: &str) -> Option<String> {
    match naraeclaw_providers::create_provider(name, None) {
        Ok(_) => None,
        Err(err) => Some(
            err.to_string()
                .lines()
...
mod tests {
    ...
    fn config_validation_warns_empty_model_route() {
        let mut config = Config::default();
        config.model_routes = vec![naraeclaw_config::schema::ModelRouteConfig {
            hint: "fast".into(),
            provider: "groq".into(),
            model: String::new(),
            api_key: None,
    ...
    fn config_validation_warns_empty_embedding_route_model() {
        let mut config = Config::default();
        config.embedding_routes = vec![naraeclaw_config::schema::EmbeddingRouteConfig {
            hint: "semantic".into(),
    ...
    fn workspace_probe_path_is_hidden_and_unique() {
        ...
        assert!(
            first
                ...
                .is_some_and(|name| name.starts_with(".naraeclaw_doctor_probe_"))
        );
    }
    ...
    fn config_validation_reports_delegate_agents_in_sorted_order() {
        ...
        config.agents.insert(
            "zeta".into(),
            naraeclaw_config::schema::DelegateAgentConfig {
