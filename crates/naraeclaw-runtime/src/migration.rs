...
fn target_memory_backend(config: &Config) -> Result<Box<dyn Memory>> {
    naraeclaw_memory::create_memory_for_migration(&config.memory.backend, &config.workspace_dir)
}
...
mod tests {
    use super::*;
    use naraeclaw_config::schema::{Config, MemoryConfig};
    use naraeclaw_memory::SqliteMemory;
    use rusqlite::params;
