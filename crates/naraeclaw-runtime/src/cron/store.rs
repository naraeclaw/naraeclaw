...
pub fn sync_declarative_jobs(
    config: &Config,
    decls: &[naraeclaw_config::schema::CronJobDecl],
) -> Result<()> {
    use naraeclaw_config::schema::CronScheduleDecl;
...
fn validate_decl(decl: &naraeclaw_config::schema::CronJobDecl) -> Result<()> {
...
fn convert_schedule_decl(decl: &naraeclaw_config::schema::CronScheduleDecl) -> Result<Schedule> {
    use naraeclaw_config::schema::CronScheduleDecl;
    match decl {
...
fn convert_delivery_decl(decl: &naraeclaw_config::schema::DeliveryConfigDecl) -> DeliveryConfig {
    DeliveryConfig {
...
mod tests {
    ...
    fn make_shell_decl(id: &str, expr: &str, cmd: &str) -> naraeclaw_config::schema::CronJobDecl {
        naraeclaw_config::schema::CronJobDecl {
            id: id.to_string(),
            name: Some(format!("decl-{id}")),
            job_type: "shell".to_string(),
            schedule: naraeclaw_config::schema::CronScheduleDecl::Cron {
                expr: expr.to_string(),
                tz: None,
            },
    ...
    fn sync_every_schedule_works() {
        ...
        let decl = naraeclaw_config::schema::CronJobDecl {
            id: "interval-job".to_string(),
            name: None,
            job_type: "shell".to_string(),
            schedule: naraeclaw_config::schema::CronScheduleDecl::Every { every_ms: 60000 },
...
schedule = { kind = "every", every_ms = 300000 }
        ...
        let parsed: naraeclaw_config::schema::CronConfig = toml::from_str(toml_str).unwrap();
        ...
        assert!(matches!(
            ...
            naraeclaw_config::schema::CronScheduleDecl::Cron { ref expr, .. } if expr == "0 9 * * *"
        ));
        ...
        assert!(matches!(
            ...
            naraeclaw_config::schema::CronScheduleDecl::Every { every_ms: 300_000 }
        ));
    }
}
