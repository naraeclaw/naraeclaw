...
pub fn show_integration_info(config: &Config, name: &str) -> Result<()> {
    ...
    let Some(entry) = entries.iter().find(|e| e.name.to_lowercase() == name_lower) else {
        anyhow::bail!(
            "Unknown integration: {name}. Check README for supported integrations or run `naraeclaw onboard` to configure channels/providers."
        );
    };
    ...
    match entry.name {
        "Telegram" => {
            ...
            println!("    3. Run: naraeclaw onboard --channels-only");
            println!("    4. Start: naraeclaw channel start");
        }
        "Discord" => {
            println!("  Setup:");
            ...
            println!("    4. Run: naraeclaw onboard --channels-only");
        }
        "Slack" => {
            println!("  Setup:");
        ...
        "OpenRouter" => {
            ...
            println!("    2. Run: naraeclaw onboard");
        ...
        "Browser" => {
            println!("  Built-in:");
            println!("    NaraeClaw can control Chrome/Chromium for web tasks.");
