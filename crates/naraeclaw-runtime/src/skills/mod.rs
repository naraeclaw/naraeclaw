...
/// Skills live in `~/.naraeclaw/workspace/skills/<name>/SKILL.md`
...
pub fn skills_to_prompt_with_mode(
    skills: &[Skill],
    workspace_dir: &Path,
    mode: naraeclaw_config::schema::SkillsPromptInjectionMode,
) -> String {
    use std::fmt::Write;
    ...
    let mut prompt = match mode {
        naraeclaw_config::schema::SkillsPromptInjectionMode::Full => String::from(
            "## Available Skills\n\n\
             Skill instructions and tool metadata are preloaded below.\n\
             Follow these instructions directly; do not read skill files at runtime unless the user asks.\n\n\
             <available_skills>\n",
        ),
        naraeclaw_config::schema::SkillsPromptInjectionMode::Compact => String::from(
            "## Available Skills\n\n\
...
) -> Vec<Box<dyn naraeclaw_api::tool::Tool>> {
    let mut tools: Vec<Box<dyn naraeclaw_api::tool::Tool>> = Vec::new();
    for skill in skills {
