...
// in `~/.naraeclaw/.secret_key` with restrictive file permissions (0600). The
...
pub struct SecretStore {
    /// Path to the key file (`~/.naraeclaw/.secret_key`)
    key_path: PathBuf,
...
impl SecretStore {
    ...
    pub fn new(naraeclaw_dir: &Path, enabled: bool) -> Self {
        Self {
            key_path: naraeclaw_dir.join(".secret_key"),
            enabled,
        }
    }
