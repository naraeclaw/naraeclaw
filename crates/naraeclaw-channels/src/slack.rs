...
/// Extract the Slack message timestamp from a NaraeClaw message ID.
///
...
impl SlackChannel {
    ...
    fn http_client(&self) -> reqwest::Client {
        naraeclaw_config::schema::build_channel_proxy_client_with_timeouts(
            "channel.slack",
    ...
    pub async fn post_message(&self, channel: &str, text: &str) -> anyhow::Result<String> {
        ...
        if !status.is_success() {
            let sanitized = naraeclaw_providers::sanitize_api_error(&raw);
    ...
    async fn fetch_sender_display_name(&self, user_id: &str) -> Option<String> {
        ...
        if !status.is_success() {
            let sanitized = naraeclaw_providers::sanitize_api_error(&body);
    ...
    fn slack_media_http_client_no_redirect(&self) -> anyhow::Result<reqwest::Client> {
        let builder = naraeclaw_config::schema::apply_channel_proxy_to_builder(
    ...
    fn parse_block_action_as_command(
        ...
        let command = match action_id {
            "naraeclaw_config_provider" => format!("/models {selected_value}"),
            "naraeclaw_config_model" => format!("/model {selected_value}"),
            _ => return None,
        };
    ...
    async fn listen_socket_mode(
        ...
        loop {
            ...
            let (ws_stream, _) = match naraeclaw_config::schema::ws_connect_with_proxy(
                &ws_url,
                "channel.slack",
                self.proxy_url.as_deref(),
            )
...
impl Channel for SlackChannel {
    ...
    async fn add_reaction(
        ...
        if !status.is_success() {
            let sanitized = naraeclaw_providers::sanitize_api_error(&text);
