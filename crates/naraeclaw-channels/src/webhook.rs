...
impl WebhookChannel {
    ...
    fn http_client(&self) -> reqwest::Client {
        naraeclaw_config::schema::build_runtime_proxy_client("channel.webhook")
    }
...
mod tests {
    ...
    fn incoming_payload_deserializes_all_fields() {
        let json = r#"{"sender": "naraeclaw_user", "content": "hello", "thread_id": "t1"}"#;
        let payload: IncomingWebhook = serde_json::from_str(json).unwrap();
        assert_eq!(payload.sender, "naraeclaw_user");
    ...
    fn outgoing_payload_serializes_content() {
        let payload = OutgoingWebhook {
            ...
            recipient: Some("naraeclaw_user".into()),
        };
        let json = serde_json::to_value(&payload).unwrap();
        assert_eq!(json["content"], "response");
        assert_eq!(json["thread_id"], "t1");
        assert_eq!(json["recipient"], "naraeclaw_user");
    }
