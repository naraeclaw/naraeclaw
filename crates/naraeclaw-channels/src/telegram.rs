...
use naraeclaw_config::schema::{Config, StreamMode};
...
pub struct TelegramChannel {
    ...
    api_base: String,
    transcription: Option<naraeclaw_config::schema::TranscriptionConfig>,
    transcription_manager: Option<std::sync::Arc<super::transcription::TranscriptionManager>>,
    voice_transcriptions: Mutex<std::collections::HashMap<String, String>>,
    workspace_dir: Option<std::path::PathBuf>,
    ack_reactions: bool,
    tts_config: Option<naraeclaw_config::schema::TtsConfig>,
...
impl TelegramChannel {
    ...
    pub fn with_transcription(
        mut self,
        config: naraeclaw_config::schema::TranscriptionConfig,
    ) -> Self {
        if !config.enabled {
            return self;
        }
    ...
    pub fn with_tts(mut self, config: naraeclaw_config::schema::TtsConfig) -> Self {
        if config.enabled {
    ...
    fn http_client(&self) -> reqwest::Client {
        naraeclaw_config::schema::build_channel_proxy_client(
            "channel.telegram",
    ...
    async fn synthesize_and_send_voice(
        ...
        text: &str,
        tts_config: &naraeclaw_config::schema::TtsConfig,
    ) -> anyhow::Result<()> {
        ...
        let client = naraeclaw_config::schema::build_runtime_proxy_client("channel.telegram");
...
Ensure only one `naraeclaw` process is using this bot token."
                    );
...
mod tests {
    ...
    fn telegram_extract_bind_code_supports_bot_mention() {
        assert_eq!(
            TelegramChannel::extract_bind_code("/bind@naraeclaw_bot 654321"),
            Some("654321")
        );
    }
    ...
    fn with_transcription_skips_when_disabled() {
        let tc = naraeclaw_config::schema::TranscriptionConfig::default(); // enabled = false
        let ch =
            TelegramChannel::new("token".into(), vec!["*".into()], false).with_transcription(tc);
    ...
    fn markdown_attachment_not_detected_by_multimodal_image_markers() {
        ...
        let messages = vec![naraeclaw_providers::ChatMessage::user(content)];
        assert_eq!(
            naraeclaw_providers::multimodal::count_image_markers(&messages),
            0,
    ...
    fn photo_image_marker_detected_by_multimodal() {
        ...
        let messages = vec![naraeclaw_providers::ChatMessage::user(
            photo_content.to_string(),
        )];
        let count = naraeclaw_providers::multimodal::count_image_markers(&messages);
        assert_eq!(
            count, 1,
    ...
    fn e2e_attachment_saves_file_and_formats_content() {
        ...
        let doc_msgs = vec![naraeclaw_providers::ChatMessage::user(doc_content)];
        assert_eq!(
            naraeclaw_providers::multimodal::count_image_markers(&doc_msgs),
            0,
        ...
        let photo_msgs = vec![naraeclaw_providers::ChatMessage::user(
            photo_content.clone(),
        )];
        assert_eq!(
            naraeclaw_providers::multimodal::count_image_markers(&photo_msgs),
            1,
        ...
        let cap_msgs = vec![naraeclaw_providers::ChatMessage::user(captioned.clone())];
        assert_eq!(
            naraeclaw_providers::multimodal::count_image_markers(&cap_msgs),
            1,
        ...
        let md_msgs = vec![naraeclaw_providers::ChatMessage::user(md_content)];
        assert_eq!(
            naraeclaw_providers::multimodal::count_image_markers(&md_msgs),
            0,
            "markdown file must not trigger image marker detection"
        );
    }
    ...
    fn groq_provider_rejects_photo_with_vision_error() {
        ...
        use naraeclaw_providers::compatible::{AuthStyle, OpenAiCompatibleProvider};
        ...
        let marker_count = naraeclaw_providers::multimodal::count_image_markers(&messages);
