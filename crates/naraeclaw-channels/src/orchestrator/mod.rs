...
//! `naraeclaw-channels/src/`, declare it in `lib.rs` behind the appropriate feature
...
// Local channel types (in misc, not naraeclaw-channels)
...
pub use naraeclaw_infra::session_sqlite::SqliteSessionBackend;
pub use naraeclaw_infra::stall_watchdog::StallWatchdog;

use anyhow::{Context, Result};
use naraeclaw_config::schema::Config;
use naraeclaw_memory::{self, Memory};
use naraeclaw_providers::reliable::{scope_provider_fallback, take_last_provider_fallback};
use naraeclaw_providers::{self, ChatMessage, Provider};
use naraeclaw_runtime::agent::loop_::{
...
use naraeclaw_runtime::approval::ApprovalManager;
use naraeclaw_runtime::observability::traits::{ObserverEvent, ObserverMetric};
use naraeclaw_runtime::observability::{self, Observer, runtime_trace};
use naraeclaw_runtime::platform;
use naraeclaw_runtime::security::{AutonomyLevel, SecurityPolicy};
use naraeclaw_runtime::tools::{self, Tool};
...
// System prompt functions live in `naraeclaw_runtime::agent::system_prompt`.
#[allow(unused_imports)]
pub use naraeclaw_runtime::agent::system_prompt::{
...
struct ChannelRuntimeDefaults {
    ...
    reliability: naraeclaw_config::schema::ReliabilityConfig,
}
...
struct ChannelCostTrackingState {
    tracker: Arc<naraeclaw_runtime::cost::CostTracker>,
    prices: Arc<HashMap<String, naraeclaw_config::schema::ModelPricing>>,
}
...
struct ChannelRuntimeContext {
    ...
    prompt_config: Arc<naraeclaw_config::schema::Config>,
    ...
    api_key: Option<String>,
    api_url: Option<String>,
    reliability: Arc<naraeclaw_config::schema::ReliabilityConfig>,
    provider_runtime_options: naraeclaw_providers::ProviderRuntimeOptions,
    workspace_dir: Arc<PathBuf>,
    message_timeout_secs: u64,
    interrupt_on_new_message: InterruptOnNewMessageConfig,
    multimodal: naraeclaw_config::schema::MultimodalConfig,
    ...
    model_routes: Arc<Vec<naraeclaw_config::schema::ModelRouteConfig>>,
    query_classification: naraeclaw_config::schema::QueryClassificationConfig,
    ack_reactions: bool,
    show_tool_calls: bool,
    session_store: Option<Arc<naraeclaw_infra::session_store::SessionStore>>,
    ...
    activated_tools:
        Option<std::sync::Arc<std::sync::Mutex<naraeclaw_runtime::tools::ActivatedToolSet>>>,
    cost_tracking: Option<ChannelCostTrackingState>,
    pacing: naraeclaw_config::schema::PacingConfig,
...
fn conversation_memory_key(msg: &naraeclaw_api::channel::ChannelMessage) -> String {
...
pub fn conversation_history_key(msg: &naraeclaw_api::channel::ChannelMessage) -> String {
...
fn followup_thread_id(msg: &naraeclaw_api::channel::ChannelMessage) -> Option<String> {
    msg.thread_ts.clone().or_else(|| Some(msg.id.clone()))
}

fn interruption_scope_key(msg: &naraeclaw_api::channel::ChannelMessage) -> String {
...
fn resolve_provider_alias(name: &str) -> Option<String> {
    ...
    let providers_list = naraeclaw_providers::list_providers();
...
fn decrypt_optional_secret_for_runtime_reload(
    store: &naraeclaw_runtime::security::SecretStore,
    value: &mut Option<String>,
    field_name: &str,
) -> Result<()> {
    if let Some(raw) = value.clone()
        && naraeclaw_runtime::security::SecretStore::is_encrypted(&raw)
    {
        *value = Some(
            store
                .decrypt(&raw)
...
async fn maybe_apply_runtime_config_update(ctx: &ChannelRuntimeContext) -> Result<()> {
    ...
    let next_default_provider = naraeclaw_providers::create_resilient_provider_with_options(
    ...
    if let Err(err) = next_default_provider.warmup().await {
        if naraeclaw_providers::reliable::is_non_retryable(&err) {
            tracing::warn!(
...
fn refreshed_new_session_system_prompt(ctx: &ChannelRuntimeContext) -> String {
    let refreshed_skills = naraeclaw_runtime::skills::skills_to_prompt_with_mode(
        &naraeclaw_runtime::skills::load_skills_with_config(
...
fn should_rollback_failed_user_turn(error: &anyhow::Error) -> bool {
    if error
        .downcast_ref::<naraeclaw_providers::ProviderCapabilityError>()
        .is_some_and(|capability| capability.capability.eq_ignore_ascii_case("vision"))
    {
        return true;
    }

    naraeclaw_providers::reliable::is_non_retryable(error)
}

fn should_skip_memory_context_entry(key: &str, content: &str) -> bool {
    if naraeclaw_memory::is_assistant_autosave_key(key) {
        return true;
    }

    if naraeclaw_memory::should_skip_autosave_content(content) {
        return true;
    }
...
async fn create_resilient_provider_nonblocking(
    ...
    tokio::task::spawn_blocking(move || {
        naraeclaw_providers::create_resilient_provider_with_options(
            &provider_name,
            api_key.as_deref(),
            api_url.as_deref(),
            &reliability,
...
fn build_models_help_response(
    ...
    workspace_dir: &Path,
    model_routes: &[naraeclaw_config::schema::ModelRouteConfig],
) -> String {
...
fn build_providers_help_response(current: &ChannelRouteSelection) -> String {
    ...
    for provider in naraeclaw_providers::list_providers() {
...
fn build_config_text_response(
    ...
    resp.push_str("\nAvailable providers:\n");
    for p in naraeclaw_providers::list_providers() {
...
fn build_config_block_kit(
    ...
    let provider_options: Vec<serde_json::Value> = naraeclaw_providers::list_providers()
        .iter()
        .map(|p| {
            serde_json::json!({
    ...
    let mut provider_select = serde_json::json!({
        "type": "static_select",
        "action_id": "naraeclaw_config_provider",
    ...
    let mut model_select = serde_json::json!({
        "type": "static_select",
        "action_id": "naraeclaw_config_model",
...
async fn handle_runtime_command_if_needed(
    ctx: &ChannelRuntimeContext,
    msg: &naraeclaw_api::channel::ChannelMessage,
            ...
            match resolve_provider_alias(&raw_provider) {
                Some(provider_name) => {
                    match get_or_create_provider(ctx, &provider_name, None).await {
                        ...
                        Err(err) => {
                            let safe_err =
                                naraeclaw_providers::sanitize_api_error(&err.to_string());
                            format!(
...
fn sanitize_channel_response(response: &str, tools: &[Box<dyn Tool>]) -> String {
    ...
    match naraeclaw_runtime::security::LeakDetector::new().scan(&sanitized) {
        naraeclaw_runtime::security::LeakResult::Clean => sanitized,
        naraeclaw_runtime::security::LeakResult::Detected { patterns, redacted } => {
            tracing::warn!(
                patterns = ?patterns,
...
fn spawn_supervised_listener(
    ch: Arc<dyn Channel>,
    tx: tokio::sync::mpsc::Sender<naraeclaw_api::channel::ChannelMessage>,
...
fn spawn_supervised_listener_with_health_interval(
    ...
    tokio::spawn(async move {
        ...
        loop {
            naraeclaw_runtime::health::mark_component_ok(&component);
            ...
            match result {
                Ok(()) => {
                    ...
                    naraeclaw_runtime::health::mark_component_error(
                        &component,
                ...
                Err(e) => {
                    ...
                    naraeclaw_runtime::health::mark_component_error(&component, e.to_string());
                }
            }

            naraeclaw_runtime::health::bump_component_restart(&component);
...
async fn process_channel_message(
    ...
    let mut msg = if let Some(hooks) = &ctx.hooks {
        match hooks.run_on_message_received(msg).await {
            naraeclaw_runtime::hooks::HookResult::Cancel(reason) => {
    ...
    if let Some(hint) =
        naraeclaw_runtime::agent::classifier::classify(&ctx.query_classification, &msg.content)
    ...
    .await
    {
        ...
        Err(err) => {
            let safe_err = naraeclaw_providers::sanitize_api_error(&err.to_string());
            let message = format!(
    ...
    if ctx.auto_save_memory
        ...
        && !naraeclaw_memory::should_skip_autosave_content(&msg.content)
    {
        ...
        let _ = ctx
            ...
            .store(
                &autosave_key,
                &msg.content,
                naraeclaw_memory::MemoryCategory::Conversation,
                Some(&history_key),
            )
            .await;
    }
    ...
    if !active_provider.supports_vision() && prior_turns.len() > 1 {
        ...
        for turn in &mut prior_turns[..last_idx] {
            if turn.content.contains("[IMAGE:") {
                let (cleaned, _refs) =
                    naraeclaw_providers::multimodal::parse_image_markers(&turn.content);
    ...
    // and preserving key decisions through LLM-driven summarization.
    {
        ...
        let compressor = naraeclaw_runtime::agent::context_compressor::ContextCompressor::new(
            cc_config,
    ...
    let (delta_tx, delta_rx) = if use_draft_streaming {
        let (tx, rx) =
            tokio::sync::mpsc::channel::<naraeclaw_runtime::agent::loop_::DraftEvent>(64);
        (Some(tx), Some(rx))
    } else {
        (None, None)
    };
    ...
    let draft_updater = if use_draft_streaming {
        ...
        if let (Some(mut rx), Some(draft_id_ref), Some(channel_ref)) = (
            ...
            Some(tokio::spawn(async move {
                use naraeclaw_runtime::agent::loop_::DraftEvent;
    ...
    let cost_tracking_context = ctx.cost_tracking.clone().map(|state| {
        naraeclaw_runtime::agent::loop_::ToolLoopCostTrackingContext::new(
            state.tracker,
            state.prices,
        )
    });
            ...
            let loop_result = tokio::select! {
                ...
                result = tokio::time::timeout(
                    ...
                    scope_thread_id(
                        ...
                        naraeclaw_runtime::agent::loop_::TOOL_LOOP_COST_TRACKING_CONTEXT.scope(
    ...
    match llm_result {
        ...
        LlmExecutionResult::Completed(Ok(Ok(response))) => {
            ...
            if let Some(hooks) = &ctx.hooks {
                    ...
                    .await
                {
                    ...
                    naraeclaw_runtime::hooks::HookResult::Continue((
                        hook_channel,
                        hook_recipient,
            ...
            if ctx.auto_save_memory && msg.content.chars().count() >= AUTOSAVE_MIN_MESSAGE_CHARS {
                ...
                tokio::spawn(async move {
                    if let Err(e) = naraeclaw_memory::consolidation::consolidate_turn(
                        provider.as_ref(),
                        &model,
                        memory.as_ref(),
                        &user_msg,
        ...
        LlmExecutionResult::Completed(Ok(Err(e))) => {
            if naraeclaw_runtime::agent::loop_::is_tool_loop_cancelled(&e)
                || cancellation_token.is_cancelled()
            {
                ...
                if naraeclaw_providers::reliable::is_auth_error(&e) {
                ...
                let safe_error = naraeclaw_providers::sanitize_api_error(&e.to_string());
                runtime_trace::record_event(
                    "channel_message_error",
                    Some(msg.channel.as_str()),
                    Some(route.provider.as_str()),
...
fn build_channel_by_id(config: &Config, channel_id: &str) -> Result<Arc<dyn Channel>> {
    match channel_id {
        ...
        "discord_history" | "discord-history" => {
            ...
            let discord_mem =
                naraeclaw_memory::SqliteMemory::new_named(&config.workspace_dir, "discord")
...
pub async fn start_channels(config: Config) -> Result<()> {
    ...
    let provider_runtime_options =
        naraeclaw_providers::provider_runtime_options_from_config(&config);
            ...
            Ok(registry) => {
                ...
                if config.mcp.deferred_loading {
                    ...
                    let activated = std::sync::Arc::new(std::sync::Mutex::new(
                        naraeclaw_runtime::tools::ActivatedToolSet::new(),
                    ));
                    ch_activated_handle = Some(std::sync::Arc::clone(&activated));
                    built_tools.push(Box::new(naraeclaw_runtime::tools::ToolSearchTool::new(
                        deferred_set,
                        activated,
                    )));
                } else {
                    ...
                    for name in names {
                        if let Some(def) = registry.get_tool_def(&name).await {
                            ...
                            built_tools
                                .push(Box::new(naraeclaw_runtime::tools::ArcToolRef(wrapper)));
                            registered += 1;
                        }
                    }
                    tracing::info!(
    ...
    let skills = naraeclaw_runtime::skills::load_skills_with_config(&workspace, &config);
    ...
    let i18n_search_dirs = naraeclaw_runtime::i18n::default_search_dirs(&workspace);
    let i18n_descs =
        naraeclaw_runtime::i18n::ToolDescriptions::load(&i18n_locale, &i18n_search_dirs);
    ...
    println!("  🤖 Model:    {model}");
    let effective_backend = naraeclaw_memory::effective_memory_backend_name(
    ...
    naraeclaw_runtime::health::mark_component_ok("channels");
    ...
    let (tx, rx) = tokio::sync::mpsc::channel::<naraeclaw_api::channel::ChannelMessage>(100);
    ...
    let runtime_ctx = Arc::new(ChannelRuntimeContext {
        ...
        hooks: if config.hooks.enabled {
            let mut runner = naraeclaw_runtime::hooks::HookRunner::new();
        ...
        session_store: if config.channels_config.session_persistence {
            match naraeclaw_infra::session_store::SessionStore::new(&config.workspace_dir) {
                Ok(store) => {
        ...
        cost_tracking: naraeclaw_runtime::cost::CostTracker::get_or_init_global(
            config.cost.clone(),
            &config.workspace_dir,
        )
...
pub async fn deliver_announcement(
    ...
    use naraeclaw_api::channel::SendMessage;

    // Scan for credential leaks before delivering
    let leak_detector = naraeclaw_runtime::security::LeakDetector::new();
    let safe_output = match leak_detector.scan(output) {
        naraeclaw_runtime::security::LeakResult::Detected { redacted, .. } => redacted,
        naraeclaw_runtime::security::LeakResult::Clean => output.to_string(),
    };
...
mod tests {
    ...
    use naraeclaw_providers::{ChatMessage, Provider};
    use naraeclaw_runtime::observability::NoopObserver;
    use naraeclaw_runtime::tools::{Tool, ToolResult};
    use std::collections::{HashMap, HashSet};
    use std::sync::Arc;
    ...
    fn pacing_config_defaults_preserve_existing_behavior() {
        let pacing = naraeclaw_config::schema::PacingConfig::default();
...
<tool_result name="http_request">{"status":200}</tool_result>"#;
    ...
    fn compact_sender_history_keeps_recent_truncated_messages() {
        ...
        let ctx = ChannelRuntimeContext {
            ...
            cost_tracking: None,
            pacing: naraeclaw_config::schema::PacingConfig::default(),
    ...
    fn rollback_orphan_user_turn_also_removes_from_session_store() {
        ...
        let store =
            Arc::new(naraeclaw_infra::session_store::SessionStore::new(tmp.path()).unwrap());
    ...
    impl Channel for TelegramRecordingChannel {
        ...
        async fn listen(
            &self,
            _tx: tokio::sync::mpsc::Sender<naraeclaw_api::channel::ChannelMessage>,
        ) -> anyhow::Result<()> {
            Ok(())
        }
...
BTC is currently around $65,000 based on latest tool output."#
            ...
            store.insert(
                ...
                RuntimeConfigState {
                    defaults: ChannelRuntimeDefaults {
                        ...
                        api_key: None,
                        api_url: None,
                        reliability: naraeclaw_config::schema::ReliabilityConfig::default(),
                    },
    ...
    impl Memory for NoopMemory {
        ...
        async fn store(
            ...
            _key: &str,
            _content: &str,
            _category: naraeclaw_memory::MemoryCategory,
        ...
        async fn recall(
            ...
            _since: Option<&str>,
            _until: Option<&str>,
        ) -> anyhow::Result<Vec<naraeclaw_memory::MemoryEntry>> {
            Ok(Vec::new())
        }

        async fn get(&self, _key: &str) -> anyhow::Result<Option<naraeclaw_memory::MemoryEntry>> {
            Ok(None)
        }

        async fn list(
            &self,
            _category: Option<&naraeclaw_memory::MemoryCategory>,
            _session_id: Option<&str>,
        ) -> anyhow::Result<Vec<naraeclaw_memory::MemoryEntry>> {
            Ok(Vec::new())
        }
    ...
    impl Memory for RecallMemory {
        ...
        async fn recall(
            ...
            Ok(vec![naraeclaw_memory::MemoryEntry {
                id: "entry-1".to_string(),
                key: "memory_key_1".to_string(),
                content: "Age is 45".to_string(),
                category: naraeclaw_memory::MemoryCategory::Conversation,
    ...
    async fn message_dispatch_processes_messages_in_parallel() {
        ...
        tx.send(naraeclaw_api::channel::ChannelMessage {
            id: "1".to_string(),
            sender: "alice".to_string(),
    ...
    fn prompt_skills_include_instructions_and_tools() {
        let ws = make_workspace();
        let skills = vec![naraeclaw_runtime::skills::Skill {
            name: "code-review".into(),
            description: "Review code for bugs".into(),
            version: "1.0.0".into(),
            author: None,
            tags: vec![],
            tools: vec![naraeclaw_runtime::skills::SkillTool {
                name: "lint".into(),
    ...
    fn channel_log_truncation_is_utf8_safe_for_multibyte_text() {
        ...
        let result =
            std::panic::catch_unwind(|| naraeclaw_runtime::util::truncate_with_ellipsis(msg, 80));
        assert!(
            result.is_ok(),
    ...
    fn full_autonomy_prompt_executes_allowed_tools_without_extra_approval() {
        let ws = make_workspace();
        let config = naraeclaw_config::schema::AutonomyConfig {
            level: naraeclaw_runtime::security::AutonomyLevel::Full,
            ..naraeclaw_config::schema::AutonomyConfig::default()
        };
    ...
    fn readonly_prompt_explains_policy_blocks_without_fake_approval() {
        let ws = make_workspace();
        let config = naraeclaw_config::schema::AutonomyConfig {
            level: naraeclaw_runtime::security::AutonomyLevel::ReadOnly,
            ..naraeclaw_config::schema::AutonomyConfig::default()
        };
        let prompt = build_system_prompt_with_mode_and_autonomy(
            ws.path(),
    ...
    fn channel_notify_observer_truncates_utf8_arguments_safely() {
        ...
        observer.record_event(
            &naraeclaw_runtime::observability::traits::ObserverEvent::ToolCallStart {
    ...
    fn conversation_memory_key_uses_message_id() {
        let msg = naraeclaw_api::channel::ChannelMessage {
            id: "msg_abc123".into(),
            sender: "U123".into(),
    ...
    fn conversation_memory_key_is_unique_per_message() {
        let msg1 = naraeclaw_api::channel::ChannelMessage {
            id: "msg_1".into(),
            sender: "U123".into(),
            reply_target: "C456".into(),
            content: "first".into(),
        ...
        let msg2 = naraeclaw_api::channel::ChannelMessage {
            id: "msg_2".into(),
            sender: "U123".into(),
            reply_target: "C456".into(),
            content: "second".into(),
...
This is an example JSON object for profile settings."#;
    ...
    fn aieos_identity_from_file() {
        use naraeclaw_config::schema::IdentityConfig;
        use tempfile::TempDir;
    ...
    async fn supervised_listener_marks_error_and_restarts_on_failures() {
        ...
        let (tx, rx) = tokio::sync::mpsc::channel::<naraeclaw_api::channel::ChannelMessage>(1);
        ...
        let snapshot = naraeclaw_runtime::health::snapshot_json();
    ...
    async fn supervised_listener_refreshes_health_while_running() {
        ...
        let first_last_ok = naraeclaw_runtime::health::snapshot_json()["components"]
        ...
        let second_last_ok = naraeclaw_runtime::health::snapshot_json()["components"]
            [&component_name]["last_ok"]
            .as_str()
            .unwrap_or("")
            .to_string();
    ...
    async fn process_channel_message_applies_query_classification_route() {
        ...
        let classification_config = naraeclaw_config::schema::QueryClassificationConfig {
            enabled: true,
            rules: vec![naraeclaw_config::schema::ClassificationRule {
                hint: "vision".into(),
        ...
        let model_routes = vec![naraeclaw_config::schema::ModelRouteConfig {
            hint: "vision".into(),
    ...
    async fn process_channel_message_classification_priority_selects_highest() {
        ...
        let classification_config = naraeclaw_config::schema::QueryClassificationConfig {
            enabled: true,
            rules: vec![
                naraeclaw_config::schema::ClassificationRule {
                    hint: "fast".into(),
    ...
    fn default_keep_tool_context_turns_is_two() {
        let config = naraeclaw_config::schema::AgentConfig::default();
