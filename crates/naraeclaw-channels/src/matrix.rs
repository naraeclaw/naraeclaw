...
impl MatrixChannel {
    ...
    fn sanitize_error_for_log(error: &impl std::fmt::Display) -> String {
        naraeclaw_providers::sanitize_api_error(&error.to_string())
    }
                    ...
                    if let Some(hinted) = self.session_owner_hint.as_ref()
                        && hinted != &whoami.user_id {
                            tracing::warn!(
                                ...
                                naraeclaw_runtime::security::redact(hinted),
                                naraeclaw_runtime::security::redact(&whoami.user_id)
                            );
                        }
                        ...
                        if let Some(whoami_device_id) = whoami.device_id.as_ref() {
                            if whoami_device_id != hinted {
                                tracing::warn!(
                                    ...
                                    naraeclaw_runtime::security::redact(hinted),
                                    naraeclaw_runtime::security::redact(whoami_device_id)
                                );
                            }
...
impl Channel for MatrixChannel {
    ...
    async fn update_draft_progress(
        ...
        if self.stream_mode == naraeclaw_config::schema::StreamMode::Partial {
