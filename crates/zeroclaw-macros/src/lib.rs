...
pub fn derive_configurable(input: TokenStream) -> TokenStream {
    ...
    // ── Zeroize-on-drop accumulator ──
    // Collects `.zeroize()` calls for every #[secret] field so that
    // the generated Drop impl clears secrets from memory when the struct
    // is freed. String, Option<String>, and Vec<String> all implement
    // zeroize::Zeroize in the zeroize 1.x crate.
    let mut zeroize_ops: Vec<proc_macro2::TokenStream> = Vec::new();
    ...
    for field in fields {
        ...
        if is_secret {
            ...
            // Zeroize: all three variants (Vec<String>, Option<String>, String)
            // implement zeroize::Zeroize in the zeroize 1.x crate.
            zeroize_ops.push(quote! {
                ::zeroize::Zeroize::zeroize(&mut self.#field_ident);
            });
    ...
    // Conditionally generate a Drop impl that zeroizes all #[secret] fields.
    // Only emitted when there is at least one #[secret] field on the struct.
    let drop_impl = if zeroize_ops.is_empty() {
        quote! {}
    } else {
        quote! {
            impl Drop for #struct_name {
                /// Clears every `#[secret]` field from memory when this struct is freed.
                /// Generated by the `Configurable` derive macro.
                fn drop(&mut self) {
                    #(#zeroize_ops)*
                }
            }
