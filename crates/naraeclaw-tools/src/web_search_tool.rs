...
impl WebSearchTool {
    ...
    fn resolve_brave_api_key(&self) -> anyhow::Result<String> {
        ...
        if let Some(ref key) = self.boot_brave_api_key
            && !key.is_empty()
            && !naraeclaw_config::secrets::SecretStore::is_encrypted(key)
        {
            return Ok(key.clone());
        }
    ...
    fn reload_brave_api_key(&self) -> anyhow::Result<String> {
        ...
        let config: naraeclaw_config::schema::Config = toml::from_str(&contents).map_err(|e| {
            anyhow::anyhow!(
        ...
        if naraeclaw_config::secrets::SecretStore::is_encrypted(&raw_key) {
            let naraeclaw_dir = self.config_path.parent().unwrap_or_else(|| Path::new("."));
            let store =
                naraeclaw_config::secrets::SecretStore::new(naraeclaw_dir, self.secrets_encrypt);
    ...
    async fn search_duckduckgo(&self, query: &str) -> anyhow::Result<String> {
        ...
        let builder =
            naraeclaw_config::schema::apply_runtime_proxy_to_builder(builder, "tool.web_search");
...
mod tests {
    ...
    fn test_resolve_brave_api_key_decrypts_encrypted_key() {
        let tmp = tempfile::TempDir::new().unwrap();
        let store = naraeclaw_config::secrets::SecretStore::new(tmp.path(), true);
