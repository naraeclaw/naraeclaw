...
use naraeclaw_config::schema::{ClassificationRule, Config, DelegateAgentConfig, ModelRouteConfig};
...
impl ModelRoutingConfigTool {
    ...
    async fn handle_set_default(&self, args: &Value) -> anyhow::Result<ToolResult> {
        ...
        {
            if naraeclaw_providers::reliable::is_non_retryable(&probe_err) {
    ...
    async fn probe_model(&self, provider_name: &str, model: &str) -> anyhow::Result<()> {
        ...
        let provider = match naraeclaw_providers::create_provider_with_url(
            provider_name,
            api_key,
