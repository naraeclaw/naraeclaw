...
use naraeclaw_channels::linq::LinqChannel;
...
#[cfg(feature = "channel-linq")]
fn linq_memory_key(msg: &naraeclaw_api::channel::ChannelMessage) -> String {
...
pub async fn run_gateway(
    ...
    // ── Hooks ──────────────────────────────────────────────────────
    let hooks: Option<std::sync::Arc<naraeclaw_runtime::hooks::HookRunner>> =
        if config.hooks.enabled {
            Some(std::sync::Arc::new(
                naraeclaw_runtime::hooks::HookRunner::new(),
            ))
        } else {
            None
        };
    ...
    let provider: Arc<dyn Provider> =
        Arc::from(naraeclaw_providers::create_resilient_provider_with_options(
    ...
    // ── Tunnel ────────────────────────────────────────────────
    let tunnel = naraeclaw_runtime::tunnel::create_tunnel(&config.tunnel)?;
    ...
    naraeclaw_runtime::health::mark_component_ok("gateway");
    ...
    let broadcast_observer: Arc<dyn naraeclaw_runtime::observability::Observer> =
        Arc::new(sse::BroadcastObserver::new(
            naraeclaw_runtime::observability::create_observer(&config.observability),
            event_tx.clone(),
            event_buffer.clone(),
        ));
    ...
    let state = AppState {
        ...
        webauthn: if config.security.webauthn.enabled {
            let secret_store = Arc::new(naraeclaw_runtime::security::SecretStore::new(
                &config.workspace_dir,
                true,
            ));
            let wa_config = naraeclaw_runtime::security::webauthn::WebAuthnConfig {
                enabled: true,
            ...
            Some(Arc::new(api_webauthn::WebAuthnState {
                manager: naraeclaw_runtime::security::webauthn::WebAuthnManager::new(
                    wa_config,
                    secret_store,
                    &config.workspace_dir,
                ),
...
async fn handle_health(State(state): State<AppState>) -> impl IntoResponse {
    let body = serde_json::json!({
        ...
        "runtime": naraeclaw_runtime::health::snapshot_json(),
    });
    Json(body)
}
...
fn prometheus_observer_from_state(
    observer: &dyn naraeclaw_runtime::observability::Observer,
) -> Option<&naraeclaw_runtime::observability::PrometheusObserver> {
    observer
        .as_any()
        .downcast_ref::<naraeclaw_runtime::observability::PrometheusObserver>()
        .or_else(|| {
            observer
                .as_any()
...
async fn run_gateway_chat_simple(state: &AppState, message: &str) -> anyhow::Result<String> {
    ...
    let system_prompt = {
        ...
        naraeclaw_runtime::agent::system_prompt::build_system_prompt(
    ...
    let prepared = naraeclaw_providers::multimodal::prepare_messages_for_provider(
        &messages,
        &multimodal_config,
    )
    .await?;
...
async fn handle_webhook(
    ...
    if state.auto_save && !naraeclaw_memory::should_skip_autosave_content(message) {
    ...
    state.observer.record_event(
        &naraeclaw_runtime::observability::ObserverEvent::AgentStart {
            provider: provider_label.clone(),
            model: model_label.clone(),
        },
    );
    state.observer.record_event(
        &naraeclaw_runtime::observability::ObserverEvent::LlmRequest {
            provider: provider_label.clone(),
            model: model_label.clone(),
            messages_count: 1,
        },
    ...
    match run_gateway_chat_simple(&state, message).await {
        Ok(response) => {
            ...
            state.observer.record_event(
                &naraeclaw_runtime::observability::ObserverEvent::LlmResponse {
                    provider: provider_label.clone(),
                    model: model_label.clone(),
                    duration,
                    success: true,
            ...
            state.observer.record_metric(
                &naraeclaw_runtime::observability::traits::ObserverMetric::RequestLatency(duration),
            );
            state.observer.record_event(
                &naraeclaw_runtime::observability::ObserverEvent::AgentEnd {
        ...
        Err(e) => {
            let duration = started_at.elapsed();
            let sanitized = naraeclaw_providers::sanitize_api_error(&e.to_string());
            ...
            state
                .observer
                .record_event(&naraeclaw_runtime::observability::ObserverEvent::Error {
...
async fn handle_linq_webhook(
    ...
    if let Some(ref signing_secret) = state.linq_signing_secret {
        ...
        if !naraeclaw_channels::linq::verify_linq_signature(
            signing_secret,
            &body_str,
            timestamp,
            signature,
...
mod tests {
    ...
    async fn metrics_endpoint_renders_prometheus_output() {
        ...
        let wrapped = sse::BroadcastObserver::new(
            Box::new(naraeclaw_runtime::observability::PrometheusObserver::new()),
            event_tx.clone(),
            event_buffer,
        );
        naraeclaw_runtime::observability::Observer::record_event(
            &wrapped,
            &naraeclaw_runtime::observability::ObserverEvent::HeartbeatTick,
        );

        let observer: Arc<dyn naraeclaw_runtime::observability::Observer> = Arc::new(wrapped);
        let state = AppState {
            config: Arc::new(Mutex::new(Config::default())),
            provider: Arc::new(MockProvider::default()),
            model: "test-model".into(),
        ...
        let body = response.into_body().collect().await.unwrap().to_bytes();
        let text = String::from_utf8(body.to_vec()).unwrap();
        assert!(text.contains("naraeclaw_heartbeat_ticks_total 1"));
    }
    ...
    async fn persist_pairing_tokens_writes_config_tokens() {
        ...
        assert!(
            naraeclaw_runtime::security::SecretStore::is_encrypted(on_disk),
