//! Wraps a node capability as a NaraeClaw [`Tool`] so it can be dispatched
...
use naraeclaw_tools::node_capabilities::requires_approval;

/// Default timeout for node invocations (30 seconds).
const NODE_INVOKE_TIMEOUT_SECS: u64 = 30;

/// A NaraeClaw [`Tool`] backed by a node capability.
///
