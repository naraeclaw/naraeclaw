...
const WS_PROTOCOL: &str = "naraeclaw.v1";
        ...
        if let Ok(parsed) = serde_json::from_str::<serde_json::Value>(text) {
            if parsed["type"].as_str() == Some("message") {
                ...
                if !content.is_empty() {
                    ...
                    if let Some(ref backend) = state.session_backend {
                        let user_msg = naraeclaw_providers::ChatMessage::user(&content);
...
async fn process_chat_message(
    state: &AppState,
    agent: &mut naraeclaw_runtime::agent::Agent,
    sender: &mut futures_util::stream::SplitSink<WebSocket, Message>,
    content: &str,
    session_key: &str,
) {
    use naraeclaw_runtime::agent::TurnEvent;
    ...
    match result {
        Ok(response) => {
            ...
            if let Some(ref backend) = state.session_backend {
                let assistant_msg = naraeclaw_providers::ChatMessage::assistant(&response);
...
mod tests {
    ...
    fn extract_ws_token_from_subprotocol() {
        ...
        headers.insert(
            ...
            "naraeclaw.v1, bearer.zc_sub456".parse().unwrap(),
        );
    ...
    fn extract_ws_token_subprotocol_with_multiple_entries() {
        ...
        headers.insert(
            "sec-websocket-protocol",
            "naraeclaw.v1, bearer.zc_tok, other".parse().unwrap(),
        );
