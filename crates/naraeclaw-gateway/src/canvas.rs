...
pub async fn handle_canvas_post(
    ...
    if !naraeclaw_runtime::tools::ALLOWED_CONTENT_TYPES.contains(&content_type) {
        return (
            ...
            Json(serde_json::json!({
                "error": format!(
                    ...
                    content_type,
                    naraeclaw_runtime::tools::ALLOWED_CONTENT_TYPES
                )
            })),
        )
            .into_response();
    ...
    if body.content.len() > naraeclaw_runtime::tools::MAX_CONTENT_SIZE {
        return (
            StatusCode::PAYLOAD_TOO_LARGE,
            Json(serde_json::json!({
                "error": format!(
                    "Content exceeds maximum size of {} bytes",
                    naraeclaw_runtime::tools::MAX_CONTENT_SIZE
                )
            })),
        )
            .into_response();
...
pub async fn handle_ws_canvas(
    ...
    const WS_CANVAS_PROTOCOL: &str = "naraeclaw.v1";
    let ws = if headers
        .get("sec-websocket-protocol")
        .and_then(|v| v.to_str().ok())
