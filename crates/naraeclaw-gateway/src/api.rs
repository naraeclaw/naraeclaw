...
pub struct CronAddBody {
    ...
    pub delivery: Option<naraeclaw_runtime::cron::DeliveryConfig>,
...
pub async fn handle_api_status(
    ...
    let health = naraeclaw_runtime::health::snapshot();
...
pub async fn handle_api_config_put(
    ...
    let incoming: naraeclaw_config::schema::Config = match toml::from_str(&body) {
        Ok(c) => c,
        Err(e) => {
            return (
...
pub async fn handle_api_cron_list(
    ...
    let config = state.config.lock().clone();
    match naraeclaw_runtime::cron::list_jobs(&config) {
...
pub async fn handle_api_cron_add(
    ...
    let config = state.config.lock().clone();
    let schedule = naraeclaw_runtime::cron::Schedule::Cron {
        expr: schedule,
        tz: None,
    };
    if let Err(e) = naraeclaw_runtime::cron::validate_delivery_config(delivery.as_ref()) {
        return (
            StatusCode::BAD_REQUEST,
    ...
    let result = if is_agent {
        ...
        let session_target = session_target
            .as_deref()
            .map(naraeclaw_runtime::cron::SessionTarget::parse)
            .unwrap_or_default();

        let default_delete = matches!(schedule, naraeclaw_runtime::cron::Schedule::At { .. });
        let delete_after_run = delete_after_run.unwrap_or(default_delete);

        naraeclaw_runtime::cron::add_agent_job(
            &config,
            name,
            schedule,
            prompt,
        ...
        naraeclaw_runtime::cron::add_shell_job_with_approval(
...
pub async fn handle_api_cron_runs(
    ...
    if let Err(e) = naraeclaw_runtime::cron::get_job(&config, &id) {
        return (
    ...
    match naraeclaw_runtime::cron::list_runs(&config, &id, limit) {
        Ok(runs) => {
...
pub async fn handle_api_cron_patch(
    ...
    let schedule = match body.schedule {
        Some(expr) if !expr.trim().is_empty() => Some(naraeclaw_runtime::cron::Schedule::Cron {
    ...
    let existing = match naraeclaw_runtime::cron::get_job(&config, &id) {
        Ok(j) => j,
        Err(e) => {
            return (
                StatusCode::NOT_FOUND,
    ...
    let is_agent = matches!(existing.job_type, naraeclaw_runtime::cron::JobType::Agent);
    ...
    let patch = naraeclaw_runtime::cron::CronJobPatch {
        name: body.name,
        schedule,
        command: patch_command,
        prompt: patch_prompt,
        ..naraeclaw_runtime::cron::CronJobPatch::default()
    };

    match naraeclaw_runtime::cron::update_shell_job_with_approval(&config, &id, patch, false) {
        Ok(job) => Json(serde_json::json!({"status": "ok", "job": job})).into_response(),
        Err(e) => (
            StatusCode::INTERNAL_SERVER_ERROR,
...
pub async fn handle_api_cron_delete(
    ...
    let config = state.config.lock().clone();
    match naraeclaw_runtime::cron::remove_job(&config, &id) {
...
pub async fn handle_api_integrations(
    ...
    let config = state.config.lock().clone();
    let entries = naraeclaw_runtime::integrations::registry::all_integrations();
...
pub async fn handle_api_integrations_settings(
    ...
    for entry in &entries {
        ...
        let enabled = matches!(
            status,
            naraeclaw_runtime::integrations::IntegrationStatus::Active
        );
        settings.insert(
...
pub async fn handle_api_doctor(
    ...
    let config = state.config.lock().clone();
    let results = naraeclaw_runtime::doctor::diagnose(&config);

    let ok_count = results
        .iter()
        .filter(|r| r.severity == naraeclaw_runtime::doctor::Severity::Ok)
        .count();
    let warn_count = results
        .iter()
        .filter(|r| r.severity == naraeclaw_runtime::doctor::Severity::Warn)
        .count();
    let error_count = results
        .iter()
        .filter(|r| r.severity == naraeclaw_runtime::doctor::Severity::Error)
        .count();
...
pub async fn handle_api_memory_list(
    ...
    if params.query.is_some() || params.since.is_some() || params.until.is_some() {
        ...
        let category = params.category.as_deref().map(|cat| match cat {
            "core" => naraeclaw_memory::MemoryCategory::Core,
            "daily" => naraeclaw_memory::MemoryCategory::Daily,
            "conversation" => naraeclaw_memory::MemoryCategory::Conversation,
            other => naraeclaw_memory::MemoryCategory::Custom(other.to_string()),
        });
...
pub async fn handle_api_memory_store(
    ...
    let category = body
        ...
        .unwrap_or(naraeclaw_memory::MemoryCategory::Core);
...
pub async fn handle_api_cli_tools(
    ...
    let tools = naraeclaw_tools::cli_discovery::discover_cli_tools(&[], &[]);
...
pub async fn handle_api_health(
    ...
    let snapshot = naraeclaw_runtime::health::snapshot();
...
fn model_route_identity_matches(
    incoming: &naraeclaw_config::schema::ModelRouteConfig,
    current: &naraeclaw_config::schema::ModelRouteConfig,
) -> bool {
...
fn embedding_route_identity_matches(
    incoming: &naraeclaw_config::schema::EmbeddingRouteConfig,
    current: &naraeclaw_config::schema::EmbeddingRouteConfig,
) -> bool {
    normalize_route_field(&incoming.hint) == normalize_route_field(&current.hint)
        && normalize_route_field(&incoming.provider) == normalize_route_field(&current.provider)
        && normalize_route_field(&incoming.model) == normalize_route_field(&current.model)
...
fn restore_model_route_api_keys(
    incoming: &mut [naraeclaw_config::schema::ModelRouteConfig],
    current: &[naraeclaw_config::schema::ModelRouteConfig],
) {
...
fn restore_embedding_route_api_keys(
    incoming: &mut [naraeclaw_config::schema::EmbeddingRouteConfig],
    current: &[naraeclaw_config::schema::EmbeddingRouteConfig],
) {
    let mut used_current = vec![false; current.len()];
    for incoming_route in incoming {
...
) -> naraeclaw_config::schema::Config {
...
fn restore_masked_sensitive_fields(
    incoming: &mut naraeclaw_config::schema::Config,
    current: &naraeclaw_config::schema::Config,
) {
...
fn hydrate_config_for_save(
    mut incoming: naraeclaw_config::schema::Config,
    current: &naraeclaw_config::schema::Config,
) -> naraeclaw_config::schema::Config {
...
pub async fn handle_claude_code_hook(
    State(state): State<AppState>,
    Json(payload): Json<naraeclaw_tools::claude_code_runner::ClaudeCodeHookEvent>,
) -> impl IntoResponse {
...
mod tests {
    ...
    use naraeclaw_runtime::security::pairing::PairingGuard;
    ...
    fn test_state(config: naraeclaw_config::schema::Config) -> AppState {
        AppState {
            ...
            web_dist_dir: None,
            canvas_store: naraeclaw_runtime::tools::CanvasStore::new(),
    ...
    fn masking_keeps_toml_valid_and_preserves_api_keys_type() {
        ...
        cfg.embedding_routes = vec![naraeclaw_config::schema::EmbeddingRouteConfig {
        ...
        let parsed: naraeclaw_config::schema::Config =
    ...
    fn hydrate_config_for_save_restores_masked_secrets_and_paths() {
        ...
        current.tunnel.ngrok = Some(naraeclaw_config::schema::NgrokTunnelConfig {
    ...
    fn hydrate_config_for_save_restores_route_keys_by_identity_and_clears_unmatched_masks() {
        ...
        incoming
            .model_routes
            .push(naraeclaw_config::schema::ModelRouteConfig {
                hint: "new".to_string(),
                provider: "openai".to_string(),
        ...
        incoming
            .embedding_routes
            .push(naraeclaw_config::schema::EmbeddingRouteConfig {
    ...
    async fn cron_api_accepts_agent_jobs() {
        ...
        let config = state.config.lock().clone();
        let jobs = naraeclaw_runtime::cron::list_jobs(&config).unwrap();
        assert_eq!(jobs.len(), 1);
        assert_eq!(jobs[0].job_type, naraeclaw_runtime::cron::JobType::Agent);
    ...
    async fn cron_api_rejects_announce_delivery_without_target() {
        ...
        let config = state.config.lock().clone();
        assert!(
            naraeclaw_runtime::cron::list_jobs(&config)
                .unwrap()
                .is_empty()
        );
    }
