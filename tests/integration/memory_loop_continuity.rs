...
use naraeclaw::memory::sqlite::SqliteMemory;
use naraeclaw::memory::traits::{Memory, MemoryCategory};
use naraeclaw::providers::ToolCall;
...
async fn compressor_with_memory_saves_summary() {
    use naraeclaw::agent::context_compressor::{ContextCompressionConfig, ContextCompressor};
...
async fn agent_survives_large_tool_output() {
    use naraeclaw::tools::{Tool, ToolResult};
...
async fn consolidation_extracts_facts_to_memory() {
    ...
    let result = naraeclaw::memory::consolidation::consolidate_turn(
        &provider,
        "test-model",
        mem.as_ref(),
...
async fn memory_survives_rapid_consolidation() {
    ...
    for i in 0..10 {
        ...
        let _ = naraeclaw::memory::consolidation::consolidate_turn(
            &provider,
            "test-model",
            mem.as_ref(),
...
async fn session_backend_persists_messages() {
    use naraeclaw::channels::session_backend::SessionBackend;
    use naraeclaw::channels::session_sqlite::SqliteSessionBackend;
    use naraeclaw::providers::traits::ChatMessage;
