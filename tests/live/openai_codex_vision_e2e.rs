...
use anyhow::Result;
use naraeclaw::providers::{ChatMessage, ChatRequest, ProviderRuntimeOptions};
...
async fn openai_codex_second_vision_support() -> Result<()> {
    ...
    let provider = naraeclaw::providers::create_provider_with_options("openai-codex", None, &opts)?;
