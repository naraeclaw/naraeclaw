...
# ── Step 1: Build ────────────────────────────────────────────────────
echo -e "${BOLD}[1/3] Building naraeclaw (${PROFILE})...${RESET}"
if [[ "$PROFILE" == "release" ]]; then
  cargo build --release 2>&1
  BIN="$REPO_ROOT/target/release/naraeclaw"
else
  cargo build 2>&1
  BIN="$REPO_ROOT/target/debug/naraeclaw"
fi
...
echo "  [ ] Welcome screen renders with NARAECLAW banner"
