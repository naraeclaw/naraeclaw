...
TAG="v${VERSION}"
ASSET_NAME="naraeclaw-${TARGET}.tar.gz"
ASSET_URL="https://github.com/naraeclaw/naraeclaw/releases/download/${TAG}/${ASSET_NAME}"
TEMP_DIR="$(mktemp -d -t naraeclaw-termux-test-XXXXXX)"
...
# --- Test 5: Contains naraeclaw binary ---
info "Checking archive contents"
CONTENTS=$(tar -tzf "$TEMP_DIR/$ASSET_NAME")
if echo "$CONTENTS" | grep -q "^naraeclaw$"; then
  pass "Archive contains 'naraeclaw' binary"
else
  fail "Archive does not contain 'naraeclaw' binary"
  echo "Contents:"
  echo "$CONTENTS" | sed 's/^/  /'
fi
...
CHECKSUMS_URL="https://github.com/naraeclaw/naraeclaw/releases/download/${TAG}/SHA256SUMS"
